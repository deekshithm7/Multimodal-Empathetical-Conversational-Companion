...
FIX: Added ToPILImage() and Resize((224, 224)) to the transform pipeline.
Raw video frames from OpenCV are arbitrary-resolution numpy arrays.
ResNet50 requires exactly (224, 224) — without the resize it crashes.
"""
...
class VisualEncoder:
    ...
    def __init__(self):
        ...
        # ToPILImage : numpy (H, W, C) uint8  → PIL Image   [handles arbitrary resolutions]
        # Resize     : any resolution          → (224, 224)  [required by ResNet50]
        # ToTensor   : PIL Image               → float [0, 1]
    ...
    def encode(self, frames: list) -> np.ndarray:
        ...
        Args:
            frames: List of np.ndarray, each (H, W, 3) RGB uint8.

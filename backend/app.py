"""
MECC Backend API v2.3 — Tri-Modal Emotion Recognition
======================================================
Changes vs v2.2:
  1. EmotionFeatureService pre-loaded at startup (WavLM-Base+, RoBERTa-large,
     ResNet50+proj256 — produces sequences for ET-TACFN).
  2. Emotion checkpoint env-var changed to MODEL_CHECKPOINT_PATH → atv_emotion.pth
  3. Personality pipeline (FeatureService + PersonalityService) is completely
     untouched — same encoders, same flow, same DB logic.
  4. Background personality task still uses the shared FeatureService (pooled
     vectors) as before.
"""
...
from services.emotion_feature_service import get_emotion_feature_service   # NEW
...
from services.feature_service         import get_feature_service            # personality only
...
async def lifespan(app: FastAPI):
    ...
    logger.info("🚀 Starting MECC Backend API v2.3…")

    try:
        init_db()

        # ── Paths / config ────────────────────────────────────────────────────
        # NEW checkpoint name — Audio+Text+Visual tri-modal model
        atv_checkpoint      = os.environ.get("MODEL_CHECKPOINT_PATH", "checkpoints/atv_emotion.pth")
        ...
        # ── Core services ─────────────────────────────────────────────────────
        emotion_service = get_emotion_service(checkpoint_path=atv_checkpoint, device=device)
        llm_service     = get_free_llm_service(model_name="llama3.2:3b")
        tts_service     = get_free_tts_service(
            provider="piper", voice="voices/en_US-lessac-medium.onnx"
        )
        ...
        # ── Emotion feature service (ET-TACFN encoders) ───────────────────────
        # Pre-load all three heavy encoders at startup so the first request
        # doesn't pay the cold-start cost.
        emotion_feat_svc = get_emotion_feature_service()
        emotion_feat_svc.preload_encoders()
        logger.info("✅ EmotionFeatureService encoders pre-loaded")

        # ── Personality feature service (unchanged) ───────────────────────────
        # WavLM-Base (768) + RoBERTa-base (768) + ResNet50 (2048) pooled vectors.
        # Only used by the background personality task — completely independent
        # of the emotion pipeline.
        feature_service = get_feature_service()
        _ = feature_service.audio_encoder
        _ = feature_service.text_encoder
        logger.info("✅ FeatureService (personality) encoders pre-loaded")
...
# ─────────────────────────────────────────────────────────────────────────────
# Background personality task  (UNCHANGED — still uses shared feature_service)
# ─────────────────────────────────────────────────────────────────────────────
...
def _run_personality_prediction_bg(
    ...
    Uses the shared FeatureService pooled vectors (768/768/2048).
...
async def send_message(
    ...
    Emotion prediction  → ET-TACFN via EmotionFeatureService (blocking, in-request)
    Personality update  → TransformerFusion via FeatureService (non-blocking background)
    """
    ...
    try:
        ...
        # Uses the SHARED feature_service (WavLM-Base/RoBERTa-base/ResNet2048)
        # which is completely separate from the emotion pipeline above.
        ...
        # audio_temp_path is kept alive — the background personality task
        # may still need it.  A scheduled cleanup job should handle old files.
        pass
...
async def end_session(
    ...
    """End a conversation session and trigger personality EMA update."""
    convo = db.query(Conversation).filter(
        Conversation.id      == uuid.UUID(conversation_id),
        Conversation.user_id == current_user.id,
    ).first()
    ...
    try:
        ...
        # ── Personality EMA update (unchanged) ────────────────────────────────
